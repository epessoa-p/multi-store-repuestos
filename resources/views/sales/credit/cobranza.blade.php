        ...
        <div class="modal-content border-0 shadow">
            ...
            <form action="{{ route('credit.payment', $sale) }}" method="POST">
                ...
                <div class="modal-body p-4">
                    ...
                    {{-- Installment select --}}
                    @php
                        $pendingInst = $sale->installments->whereIn('status', ['pending','partial','overdue'])->sortBy('number');
                    @endphp
                    @if($pendingInst->isNotEmpty())
                    <div class="mb-3">
                        <label class="form-label fw-semibold" for="inst_{{ $sale->id }}">Aplicar a cuota</label>
                        <select name="sale_installment_id" id="inst_{{ $sale->id }}" class="form-select">
                            <option value="">Distribuir automáticamente</option>
                            @foreach($pendingInst as $inst)
                            <option value="{{ $inst->id }}">
                                Cuota #{{ $inst->number }} — Vence {{ $inst->due_date->format('d/m/Y') }} — Saldo ${{ number_format($inst->balance, 2) }}
                                @if($inst->is_overdue) [VENCIDA] @endif
                            </option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="row g-3 mb-3">
                        ...
                        <div class="col-md-6">
                            <label class="form-label fw-semibold" for="ref_{{ $sale->id }}">Referencia</label>
                            <input type="text" name="reference" id="ref_{{ $sale->id }}"
                                   class="form-control"
                                   value="{{ old('reference') }}"
